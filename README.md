...
│   └── Flowchart.jpg
│
├── models/                   # Model weights — not tracked by git (see .gitignore)
│   ├── whisper-base/         # Download separately; see Getting Started
│   └── en_US-lessac-low.onnx
│
└── data/                     # Place your PDF files here
```

> **Note:** Model files in `models/` are excluded from version control due to their size. See [Getting Started](#-getting-started) for download instructions.
...
`setup.sh` will:
1. Install system dependencies (`ffmpeg`, `ollama`, etc.)
2. Create and activate a Python virtual environment
3. Install all Python dependencies from `requirements.txt`
4. Pull and build the Ollama LLM model
5. Download Piper TTS model weights
6. Install and enable all `systemd` services with the correct paths for your machine
...
#### 3. Build the Custom Ollama Model
...
```bash
# Whisper base (INT8) — via faster-whisper / Hugging Face
...
The install script automatically replaces hardcoded paths with your actual install directory:

```bash
INSTALL_DIR=$(pwd)
for svc in systemd/*.service; do
    name=$(basename "$svc")
    sed "s|/opt/edge-ai-tutor|$INSTALL_DIR|g" "$svc" > /tmp/"$name"
